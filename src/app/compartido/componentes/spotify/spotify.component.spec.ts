...
describe('SpotifyComponent', () => {
  ...
  beforeEach(() => {
    ...
    spotifyService.isTokenExpired.and.returnValue(false);
    spotifyService.getAccessToken.and.returnValue('token123');
    spotifyService.transferPlaybackHere.and.returnValue(of({}));
    spotifyService.getCurrentPlayback.and.returnValue(Promise.resolve({}));
  ...
  it('Debería inicializar sin token o expirado y no llamar al SDK', () => {
    spotifyService.isTokenExpired.and.returnValue(true);
    component.ngOnInit();
  ...
  it('Debería cargar el SDK y crear el reproductor', fakeAsync(() => {
    ...
    expect(document.body.appendChild).toHaveBeenCalledWith(fakeScript);
  ...
  it('Debería transferir la reproducción al dispositivo indicado', () => {
    component['player'] = window.Spotify.Player();
    spotifyService.transferPlaybackHere.calls.reset();
    spotifyService.transferPlaybackHere.and.returnValue(of({}));

    spotifyService.transferPlaybackHere('deviceId').subscribe();
    expect(spotifyService.transferPlaybackHere).toHaveBeenCalledWith('deviceId');
  });
  ...
  it('Debería alternar play y pausa correctamente', fakeAsync(() => {
    ...
    (component['player'].getCurrentState as jasmine.Spy)
      .and.returnValue(Promise.resolve({ paused: false }));
    component.togglePlay();
    tick();
    expect(component['player'].pause).toHaveBeenCalled();
  }));

  it('Debería ir a anterior y siguiente canción', fakeAsync(() => {
    component.isReady = true;
    component['player'] = window.Spotify.Player();
