...
describe('CalibracionCamaraComponent', () => {
    ...
    describe('startCam', () => {
        it('debería alertar si no hay getUserMedia', fakeAsync(() => {
            spyOnProperty(navigator, 'mediaDevices', 'get').and.returnValue(undefined as any);
            spyOn(window, 'alert');
        ...
        it('debería alertar si hay error no-AbortError', fakeAsync(() => {
            const mediaDevicesMock = {
                getUserMedia: jasmine.createSpy('getUserMedia').and.returnValue(Promise.reject({ name: 'OtherError' }))
            } as unknown as MediaDevices;

            spyOnProperty(navigator, 'mediaDevices', 'get').and.returnValue(mediaDevicesMock);
            spyOn(window, 'alert');
    ...
    it('drawPose no dibuja si no hay poses', () => {
        const ctx = (component as any).ctx as jasmine.SpyObj<CanvasRenderingContext2D>;
        component.drawPose([]);
    ...
    it('drawPose dibuja cada keypoint correctamente', () => {
        const ctx = (component as any).ctx as jasmine.SpyObj<CanvasRenderingContext2D>;
        const pose = { keypoints: [{ x: 1, y: 2 }, { x: 3, y: 4 }] } as any;
    ...
    it('volverARutina detiene cámara y navega', () => {
        ...
        expect(component.detenerCamara).toHaveBeenCalled();
        expect(mockRouter.navigate).toHaveBeenCalledWith(['/informacion-ejercicio']);
    });
    ...
    it('ngOnDestroy detiene cámara y remueve listener', () => {
        ...
        expect(component.detenerCamara).toHaveBeenCalled();
        expect(window.removeEventListener).toHaveBeenCalledWith('resize', (component as any).handleResize);
    });
    ...
    describe('detectPoseLoop', () => {
        ...
        it('estima pose, dibuja y agenda siguiente frame', fakeAsync(() => {
            ...
            expect(detSpy.estimatePoses).toHaveBeenCalledWith(component.videoRef.nativeElement);
